...
MetaMask: Browser extension with an account funded with Sepolia ETH.
Visual Studio Code: With the Live Server plugin installed.
Hardhat: For contract compilation and deployment (optional if not redeploying).
...
Install Dependencies (if redeploying the contract):
...
Optional: Redeploy the Contract:
